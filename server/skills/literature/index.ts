...
export class LiteratureSkill implements Skill {
  ...
  private async searchPapers(input: Record<string, unknown>): Promise<string> {
    ...
    try {
      ...
      const entries: any[] = [];
      const entryRegex = /<entry>([sS]*?)</entry>/g;
      let match;
      while ((match = entryRegex.exec(text)) !== null && entries.length < maxResults) {
        const entry = match[1];
        const title = entry.match(/<title>([sS]*?)</title>/)?.[1]?.replace(/\n/g, ' ').trim() || '';
        const authors: string[] = [];
        const authorRegex = /<name>([sS]*?)</name>/g;
        let am;
        while ((am = authorRegex.exec(entry)) !== null) authors.push(am[1].trim());
        const summary = entry.match(/<summary>([sS]*?)</summary>/)?.[1]?.replace(/\n/g, ' ').trim() || '';
        const link = entry.match(/<id>([sS]*?)</id>/)?.[1]?.trim() || '';
        const published = entry.match(/<published>([sS]*?)</published>/)?.[1]?.trim();
