...
app.post("/api/chat", async (req, res) => {
  ...
  // 默认系统提示词（可被环境变量覆盖，前端 agent 配置会覆盖此默认值）
  ...
  // 工作目录：优先使用请求中的 cwd，其次环境变量 DEFAULT_CWD，最后当前目录
  const workingDir = cwd || process.env.DEFAULT_CWD || process.cwd();

  // 权限模式：优先请求参数，其次环境变量 DEFAULT_PERMISSION_MODE，最后 default
  ...
  try {
    ...
    console.log(`[Chat] - PermissionMode: ${pm}`);
...
// 生产环境：若构建产物 dist/ 存在，则由其托管前端静态资源（单进程部署）
const distDir = path.join(__dirname, "..", "dist");
if (fs.existsSync(distDir)) {
  console.log(`[Server] 生产模式：托管前端静态资源 -> ${distDir}`);
  app.use(express.static(distDir));
  // 兜底路由：非 /api 请求全部返回 index.html（支持前端 SPA 路由）
  app.get(/^(?!\/api).*/, (req, res) => {
